...
const schema = z.object({
  bed_number: z.string().min(1, "*Required"),
});
...
export default function ManageBeds() {
  ...
  const updateBed = async (id, hostel_id, room_id, status) => {
    setIsProcessing(true);
    try {
      const response = await fetch(
        VITE_API_URL + "/api/edit-bed/" + id + "/" + hostel_id + "/" + room_id,
        {
          mode: "cors",
          headers: {
            Authorization: `Bearer ${
  ...
  const submit = async (event) => {
    ...
    try {
      const response = await fetch(VITE_API_URL + "/api/create-bed", {
        mode: "cors",
        headers: {
          Authorization: `Bearer ${
            JSON.parse(localStorage.getItem("jetsUser")).token
  ...
  const [beds] = createResource(fetchBeds);
  return (
    <MetaProvider>
      <Title>Manage Bedspaces - ECWA Theological Seminary, KAGORO (ETSK)</Title>
      <Meta
        name="description"
        content="Manage Bedspaces on ECWA Theological Seminary, KAGORO (ETSK)"
      />
      <div class="text-sm">
        <Show when={showModal()}>
        ...
        <div class="mt-8 w-11/12 mx-auto space-y-4">
          <h2 class="text-lg font-semibold text-center border-b border-red-600">
            Bedspaces / {decodeURI(params.roomNumber)} /{" "}
            {decodeURI(params.hostelName)}
          </h2>
          <div class="bg-yellow-100 rounded-md border border-yellow-200  p-1 space-y-0.5">
            <b class="block">Instruction:</b>
            <p>Create and manage Bedspaces here.</p>
          </div>
                      ...
                      <For each={beds()}>
                        {(bed, i) => (
                          <tr class="even:bg-gray-200 odd:bg-white even:border-y border-black">
                            ...
                            <td class="p-4 border-r border-black">
                              Bebspace/Bunk <b>{bed.bed_number}</b>
                            </td>
                            <td class="p-4">
                              <b class="uppercase">{bed.status}</b>{" "}
